...
develop<-ggscatter(cats, x = "mass.mg.", y = "ug.vol.correct",
  ...
  panel_border() +
  xlab('Larval mass (mg)') +
  ylab(expression(paste("Cyanide (", mu, "g)"))) +
...
allCNs<-ggplot(comparison, aes(x=host, y=ug.total,fill=life.stage)) + 
  ...
  ggtitle("") +
  xlab("Host plant") + 
  ylab(expression(paste("Cyanide content (", mu, "g)"))) +
...
palatable<-lme(ug.vol.correct~pair,weights=varIdent(form=~1|as.factor(pair)),
               random = ~1|number, method = "REML",data=hc)
shapiro.test(resid(palatable)) # W = 0.95985, p-value = 0.04628 --> normally distributed

# - plot of normal distribution
plot(hist(resid(palatable),breaks = 6),
     main="(a) LMM: CN (ug total) ~ host, random = ~1|individuals",
     xlab="residuals",ylab="frequency",
     xlim=c(-600,700))
text(x=550,y=16,label=expression(bold(paste("Shapiro-Wilk"))))
text(x=550,y=15,label=expression(paste(italic('P'), " = 0.05")))
text(x=550,y=14,label="W = 0.96")
...
CONpalatable<-lme(ug.mg~pair,weights=varIdent(form=~1|as.factor(pair)),
                  random = ~1|number, method = "REML",data=hc2)
shapiro.test(resid(CONpalatable)) # W = 0.93966, p-value = 0.028 --> approaching normal distribution

# - plot of normal distribution
plot(hist(resid(CONpalatable),breaks = 6),
     main="(b) LMM: CN (ug/mg) ~ host, random = ~1|individuals",
     xlab="residuals",ylab="frequency")
text(x=4.5,y=14.75,label=expression(bold("Shapiro-Wilk")))
text(x=4.5,y=13.75,label=expression(paste(italic('P'), " = 0.02")))
text(x=4.5,y=12.75,label="W = 0.94")


################################################################################## 
### ---/// SUPPLEMENTAL FIGURE 2: Larval & adult cyanide concentration  ///--- ###
##################################################################################
# --- BARGRAPH of adult CN concentration on each host 
# using summary stats on amount of adult CN per host association
adult<-read.csv("char.adult.csv")
hc<-adult[,c('species','pair','ug','ug.vol.correct','ug.mg')]
head(hc)
...
############################################################################################### 
### ---/// SUPPLEMENNTAL FIGURE 3: Adult (+ P. suberosa) CN content & concentration  ///--- ###
###############################################################################################
adult<-read.csv("char.adult.csv") # 
hc<-adult[,c('species','pair','ug','ug.vol.correct','ug.mg')]
